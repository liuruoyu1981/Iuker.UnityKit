...
namespace Jint.Runtime.Interop
{
    public class DefaultTypeConverter : ITypeConverter
    {
            ...
            if (valueType == typeof(Func<JsValue, JsValue[], JsValue>))
            {
                ...
                if (type.IsGenericType)
                {
                    ...
                    if (genericType.Name.StartsWith("Action"))
                    {
                        ...
                        var callExpresion = Expression.Block(Expression.Call(
                            Expression.Call(Expression.Constant(function.Target),
                                function.Method,
                                // todo myfixed function.GetMethodInfo(),
                                Expression.Constant(JsValue.Undefined, typeof(JsValue)),
                                @vars),
                            jsValueToObject), Expression.Empty());

                        var parameters = new ReadOnlyCollection<ParameterExpression>(@params).ToArray();
                        return Expression.Lambda(callExpresion, parameters).Compile();
                    }
                    else if (genericType.Name.StartsWith("Func"))
                    {
                        var genericArguments = type.GetGenericArguments();
                        ...
                        var @vars = Expression.NewArrayInit(typeof(JsValue),
                            (System.Linq.Expressions.Expression[])@params.Select(p =>
                            {
                                var boxingExpression = Expression.Convert(p, typeof(object));
                                return Expression.Call(null, jsValueFromObject, Expression.Constant(_engine, typeof(Engine)), boxingExpression);
                            })
                ...
                else
                {
                    ...
                    else if (typeof(MulticastDelegate).IsAssignableFrom(type))
                    {
                        ...
                        var initializers = @params.Select(p => Expression.Call(null, typeof(JsValue).GetMethod("FromObject"), Expression.Constant(_engine, typeof(Engine)), p));

                        var @vars = Expression.NewArrayInit(typeof(JsValue), initializers.ToArray());
        ...
        protected class Expression : System.Linq.Expressions.Expression
        {
            ...
            public static MethodCallExpression Block(System.Linq.Expressions.Expression expression, System.Linq.Expressions.Expression parameter)
            {
