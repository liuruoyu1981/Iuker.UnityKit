...
using Iuker.Js.Runtime.References;
...
namespace Iuker.Js.Runtime
{
    ...
    public class TypeConverter
    {
        ...
        /// 转换为原始类型
        /// </summary>
        /// <param name="input">待转换的目标Js对象</param>
        /// <param name="preferredType">优先类型</param>
        /// <returns></returns>
        ...
        public static bool ToBoolean(JsValue o)
        {
            ...
            {
                var n = o.AsNumber();
                return !n.Equals(0) && !double.IsNaN(n);
            }
        ...
        {
            methods = methods
                .Where(m => m.GetParameters().Count() == arguments.Length).ToArray();
        ...
        public static bool TypeIsNullable(Type type) => !type.IsValueType() || Nullable.GetUnderlyingType(type) != null;
    }
}
