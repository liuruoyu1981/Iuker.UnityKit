...
***特别说明:由于3.0版之后URL重写规则命令变化较大,请自行修改旧规则***
    ...
    简单使用说明:
　　使用FastCGI时，有两种途径:1.如果使用fastcgi_mod.dll模块(不建议使用此模式),将扩展映射设置为PHP-CGI.exe路径即可;2.如果使用内置fastcgi进程管理器,先配置好进程管理后,将映射模块设置为对应的"IP:端口号"（如：127.0.0.1:888）。
...
　　sethd  修改反向代理时发送的HTTP头值 格式为 sethd=头名称:头值，头名称区分大小写。
    ...
    特别声明:标准版软件为免费软件（不提供任何技术支持），但不得用于商业用途，不得修改和反汇编程序文件。软件包中的所有DLL文件为第三方软件部件,文件来源于网络,不属于本软件的一部分,也不保证其安全性和稳定性,仅作为测试使用,不对其产生的后果负责.

    需定制软件或有任何建议和意见请联系 175384354@qq.com
